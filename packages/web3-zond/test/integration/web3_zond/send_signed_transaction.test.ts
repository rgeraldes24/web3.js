...
import { Wallet, Web3Account } from '@theqrl/web3-zond-accounts';
...
describe('Web3Zond.sendSignedTransaction', () => {
	...
	describe('Transaction Types', () => {
		...
		*/
		...
		it('should send a signed simple value transfer - type 0x2', async () => {
			const temp = await createTempAccount();
			wallet.add(temp.seed);
			const acc = wallet.get(temp.address);
			if (acc === undefined) {
			...
			// });
			// const response = await web3Zond.sendSignedTransaction(signedTransaction.raw);
			const response = await web3Zond.sendSignedTransaction(rawTransaction);
			expect(response.status).toBe(BigInt(1));
	...
	// TODO(rgeraldes24): expected address checksum
	it.skip('should send a signed contract deployment', async () => {
		...
		const { rawTransaction } = await tempAcc.signTransaction({ ...transaction, ...gasPricing })
		...
		const response = await web3Zond.sendSignedTransaction(rawTransaction as Bytes);
	...
	describe('Transaction PromiEvents', () => {
		...
		beforeEach(async () => {
			...
			const { rawTransaction } = await tempAcc.signTransaction({ ...transaction, ...gasPricing });
			rawSignedTx = rawTransaction 
		});

		it('should listen to the sending event', async () => {
			await web3Zond.sendSignedTransaction(rawSignedTx).on('sending', data => {
				expect(data).toBe(rawSignedTx);
			});
			expect.assertions(1);
		});

		it('should listen to the sent event', async () => {
			await web3Zond.sendSignedTransaction(rawSignedTx).on('sent', data => {
				expect(data).toBe(rawSignedTx);
			});
			expect.assertions(1);
		});
		...
		it('should listen to the transactionHash event', async () => {
			await web3Zond
				.sendSignedTransaction(rawSignedTx)
		...
		// TODO(rgeraldes24): different result: address checksum
		it.skip('should listen to the receipt event', async () => {
			...
			await web3Zond.sendSignedTransaction(rawSignedTx).on('receipt', data => {
		...
		it('should listen to the confirmation event', async () => {
			...
			await web3Zond.sendSignedTransaction(rawSignedTx).on('confirmation', data => {
	...
	describe('Transaction Error Scenarios', () => {
		...
		*/
		...
		it('Should throw TransactionRevertInstructionError because of contract revert and return revert reason', async () => {
			...
			// 	bytes: FMT_BYTES.UINT8ARRAY,
			// });
			const { rawTransaction } = await tempAcc.signTransaction(transaction);
		...
		it('Should throw TransactionRevertWithCustomError because of contract revert and return custom error ErrorWithNoParams', async () => {
			const transaction: Transaction = {
				...
				maxFeePerGas: BigInt(108571383800),
				maxPriorityFeePerGas: BigInt(25415778028),
				// TODO(rgeraldes24): might be related to the fork changes
				// gas: 21222,
				gas: 21272,
				nonce: await web3Zond.getTransactionCount(tempAcc.address),
			};
			...
			await expect(
				web3Zond
					.sendSignedTransaction(rawTransaction, undefined, {
